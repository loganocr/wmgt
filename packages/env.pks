...
begin
  ...
  select count(*)
   into l_cards_import_present
  from  user_tables
  where table_name = 'WMG_CARD_RUNS';
...
cards constant boolean := ' || case when l_cards_import_present = 1 then 'true' else 'false' end || ';
