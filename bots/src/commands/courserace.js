...
const raceLeaderboardService = new RaceLeaderboardService();
const commandLogger = logger.child({ command: 'courserace' });
const errorHandler = new ErrorHandler(commandLogger);

export default {
  data: new SlashCommandBuilder()
    .setName('courserace')
    .setDescription('Display race leaderboard times for a selected course')
  ...
  async execute(interaction) {
    try {
      ...
      commandLogger.info('Course race command executed', {
        userId: userId,
      ...
      const loadingEmbed = new EmbedBuilder()
        .setColor(0x0099FF)
        .setTitle('⏳ Fetching Race Leaderboard...')
        .setDescription(`Please wait while we retrieve the race leaderboard for course **${courseCode.toUpperCase()}**.`);
      ...
      let leaderboardData;
      try {
        const apiResponse = await raceLeaderboardService.getRaceLeaderboard(courseCode, userId);
        leaderboardData = raceLeaderboardService.formatLeaderboardData(apiResponse, userId);

        commandLogger.debug('Race leaderboard data retrieved successfully', {
          courseCode: courseCode,
        ...
        commandLogger.error('Failed to fetch race leaderboard', {
          error: error.message,
          stack: error.stack,
          userId: userId,
          courseCode: courseCode,
        ...
        if (error.errorType === 'COURSE_NOT_FOUND' && error.suggestions) {
          ...
          errorEmbed = new EmbedBuilder()
            ...
            .addFields({
              name: '🔄 What to do next',
              value: 'Please try the /courserace command again in a moment.',
              inline: false
            })
          ...
          const processedError = raceLeaderboardService.handleApiError(error, 'race_command_fetch');

          errorEmbed = new EmbedBuilder()
            .setColor(0xFF0000)
            .setTitle('❌ Unable to Fetch Race Leaderboard')
      ...
      if (!leaderboardData.entries || leaderboardData.entries.length === 0 || leaderboardData.isEmpty) {
        const noDataEmbed = new EmbedBuilder()
          .setColor(0xFFA500)
          .setTitle('📊 No Race Times Available')
          .setDescription(leaderboardData.message || `No race times have been recorded for course **${leaderboardData.course.name}** yet.`)
          .addFields({
            name: 'Course Information',
            value: `**Code:** ${leaderboardData.course.code}\n**Difficulty:** ${leaderboardData.course.difficulty}`,
            inline: false
          })
          .setFooter({ text: 'Be the first to race this course and set a time!' });
      ...
      try {
        const embedData = raceLeaderboardService.createLeaderboardEmbed(leaderboardData);
        ...
        commandLogger.info('Race leaderboard embed created successfully', {
          courseCode: courseCode,
        ...
        commandLogger.error('Failed to create race leaderboard embed', {
          error: error.message,
          stack: error.stack,
          userId: userId,
          courseCode: courseCode,
      ...
      try {
        if (!fallbackToText) {
          ...
          const textDisplay = raceLeaderboardService.createTextDisplay(leaderboardData);
          await interaction.editReply({
            content: textDisplay,
            embeds: [] // Clear any existing embeds
          });

          commandLogger.info('Successfully sent text-based race leaderboard', {
            userId: userId,
            courseCode: courseCode,
        ...
        commandLogger.error('Failed to send race leaderboard (both embed and text)', {
        ...
        const errorMessage = `❌ **Race Leaderboard Unavailable**\n\nUnable to display race leaderboard for course **${courseCode.toUpperCase()}** due to a technical issue. Please try again later or contact support.`;
      ...
      commandLogger.info('Course race command completed successfully', {
        userId: userId,
        courseCode: courseCode,
      ...
      commandLogger.error('Unexpected error in courserace command', {
        error: error.message,
        stack: error.stack,
      ...
      await errorHandler.handleInteractionError(error, interaction, 'courserace_command');
    }
  },
  ...
  async autocomplete(interaction) {
    try {
      ...
      commandLogger.debug('Course race autocomplete requested', {
        userId: interaction.user.id,
      ...
      let courses;
      try {
        courses = await raceLeaderboardService.getAvailableCourses();
      } catch (error) {
        ...
        courses = raceLeaderboardService.getFallbackCourses();
      ...
      commandLogger.debug('Course race autocomplete results prepared', {
        userId: interaction.user.id,
        focusedValue: focusedValue,
      ...
      commandLogger.error('Error in courserace autocomplete', {
        error: error.message,
        stack: error.stack,
        userId: interaction.user.id,
